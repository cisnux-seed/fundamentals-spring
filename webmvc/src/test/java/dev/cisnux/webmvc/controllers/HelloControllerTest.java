...
class HelloControllerTest {
    ...
    void helloName() throws Exception {
        mockMvc.perform(get("/hello").queryParam("name", "cisnux")).andExpectAll(status().isOk(), content().string(Matchers.containsString("Hello cisnux")));
    }

    @Test
    void helloPost() throws Exception {
        mockMvc.perform(
                post("/hello").queryParam("name", "Fajra")
        ).andExpectAll(
    ...
    void helloView() throws Exception {
        mockMvc.perform(
                get("/web/hello").queryParam("name", "cisnux")
        ).andExpectAll(
                status().isOk(),
